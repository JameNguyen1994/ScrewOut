...
public class BuyBundleHandler : IPurchaseHandler
{
    Transform tfmCoin;
    ...
    public void OnPurchaseSuccess(string productID, object data)
    {
        ...
        var addHold = coinData.data.Find(x => x.resourceType == ResourceType.ADD_HOLE).value;
        var hammer = coinData.data.Find(x => x.resourceType == ResourceType.HAMMER).value;
        var clear = coinData.data.Find(x => x.resourceType == ResourceType.CLEAR).value;
