...
public class BuyClearHandler : IPurchaseHandler
{
    Transform tfmCoin;
    ...
    async UniTask ActionAfterBuy(string productID, IAPItemData data)
    {
        ...
        Debug.Log($"Get {clear} unlock box  productID: {productID}");
        ...
        if (clear > 0)
        {
            GameAnalytics.NewResourceEvent(GAResourceFlowType.Source, "AddHole", clear, "iap", productID);
            lstResource.Add(new ResourceIAP.ResourceValue()
            {
