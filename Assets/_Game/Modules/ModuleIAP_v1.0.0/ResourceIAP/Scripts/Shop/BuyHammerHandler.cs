...
public class BuyHammerHandler : IPurchaseHandler
{
    Transform tfmCoin;
    ...
    async UniTask ActionAfterBuy(string productID, IAPItemData data)
    {
        ...
        Debug.Log($"Get {hammer} unlock box  productID: {productID}");
        ...
        if (hammer > 0)
        {
            GameAnalytics.NewResourceEvent(GAResourceFlowType.Source, "AddHole", hammer, "iap", productID);
            lstResource.Add(new ResourceIAP.ResourceValue()
            {
