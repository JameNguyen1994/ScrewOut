...
{
    public class ItemTabLegend : MonoBehaviour
    {
        ...
        public void SetData(int year,int month, List<UserData> lstUserData,int indexPlayer)
        {
            if (txtTime != null)
            {
                txtTime.text = $"{year}/{month}";
            }
            var user1 = lstUserData[0];

            var user2 = lstUserData[1];
            var user3 = lstUserData[2];
