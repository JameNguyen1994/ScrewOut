...
namespace AppLovinMax.Scripts.IntegrationManager.Editor
{
    ...
    public abstract class AppLovinProcessGradleBuildFile : AppLovinPreProcess
    {
        ...
        protected static void AddAppLovinQualityServicePlugin(string applicationGradleBuildFilePath)
        {
            ...
            {
                MaxSdkLogger.UserError("Failed to install AppLovin Quality Service plugin. SDK KeyUnlockBox is empty. Please enter the AppLovin SDK KeyUnlockBox in the Integration Manager.");
                return;
            }

            // Retrieve the API KeyUnlockBox using the SDK KeyUnlockBox.
            var qualityServiceData = RetrieveQualityServiceData(sdkKey);
            var apiKey = qualityServiceData.api_key;
            if (string.IsNullOrEmpty(apiKey))
            {
                MaxSdkLogger.UserError("Failed to install AppLovin Quality Service plugin. API KeyUnlockBox is empty.");
                return;
            }
        ...
        private static AppLovinQualityServiceData RetrieveQualityServiceData(string sdkKey)
        {
            ...
            using (var unityWebRequest = new UnityWebRequest("https://api2.safedk.com/v1/build/cred"))
            {
...
#endif
                {
                    MaxSdkLogger.UserError("Failed to retrieve API KeyUnlockBox for SDK KeyUnlockBox: " + sdkKey + "with error: " + unityWebRequest.error);
                ...
                {
                    MaxSdkLogger.UserError("Failed to parse API KeyUnlockBox." + exception);
                    return new AppLovinQualityServiceData();
                }
            }
        }
        ...
        private static List<string> GenerateUpdatedBuildFileLines(List<string> lines, string apiKey, bool addBuildScriptLines)
        {
            ...
            // ...
            var outputLines = new List<string>();
            // Check if the plugin exists, if so, update the SDK KeyUnlockBox.
